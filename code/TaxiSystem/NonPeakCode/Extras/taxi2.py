...
other_taxi_ip = ["5000","5002"]
url_taxi = "http://0.0.0.0:"

taxi = {'id': 2, 'currentLoc': [41.898331794,-87.620762865], 'noPassengers':0, 'passenger':[], 'direction':[]}
route = []
cost = 0
...
def updateTaxi():
    ...
    for i in taxi["passenger"]:
        i["elapsedTime"] += taxi["direction"][0]["time"]
...
def broadcastToAllPeers(data):
    ...
    for i in other_taxi_ip:
        ...
        if(int(s['distance']< 3000)):
            ...
            costs.append(r['cost'])
    if(len(costs)==0):
        return({'cost':-1,'id' : taxi['id'], 'port':5001})
...
def pingRequest():
    ...
    return(json.dumps({'port': 5001, 'distance': dist}))
...
def processRequest():
    ...
    retVal = {'id': taxi['id'], 'port':5001, 'cost': cost}
