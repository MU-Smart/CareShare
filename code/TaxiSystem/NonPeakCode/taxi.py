...
def processRequest():
    ...
    request_queue.append({"timestamp":datetime.datetime.now().strftime("%H:%M:%S"),"type":"getRequest"})
    ...
    if(request_in_process["status"]==True):
         return (json.dumps({'id':taxi['id'], 'port': int(taxi_id), 'cost':-1,'message':"1.waiting for others pass confirmation"}))


    if(taxi['noPassengers'] >= 4):
        if 'first_hop' in request.args:
             return (json.dumps({'id':taxi['id'], 'port': int(taxi_id), 'cost':-1,"message":"2.capacity"}))
        else:
    ...
    retVal = {'id': taxi['id'], 'port':int(taxi_id), 'cost': recv_cost,'detour_obj':0,'time_obj':0}
    ...
    #return value to passenger
    retVal = {'id': taxi['id'], 'port':int(taxi_id), 'cost': cost,"detour_obj":detour_distance,"time_obj":dataRecv['cost_time']}
...
def confirmRequest():
    ...
    request_queue.append({"timestamp":datetime.datetime.now().strftime("%H:%M:%S"),"type":"requestConfirm"})
    ...
    return json.dumps({'reach_time':reach_time, 'pwt':passenger_waiting_time,'passenger_no':taxi['noPassengers'],'taxi_id':taxi['id']})
...
def pheromone_update():
    ...
    request_queue.append({"timestamp":datetime.datetime.now().strftime("%H:%M:%S"),"type":"broadcastPheromone"})
...
def find_taxi_metric():
	...
	process_info = psutil.Process().as_dict(attrs=['connections', 'cpu_affinity', 'cpu_num', 'cpu_percent', 'cpu_times', 'create_time', 'cwd', 'environ', 'exe', 'gids', 'io_counters', 'ionice', 'memory_full_info', 'memory_info', 'memory_maps', 'memory_percent', 'name', 'nice', 'num_ctx_switches', 'num_fds', 'num_threads', 'open_files', 'pid', 'ppid', 'status', 'terminal', 'threads', 'uids', 'username'])
	process_info_cpuP = psutil.Process().cpu_percent(None)


	sys_val = {'cpuTimes':cpuTimes,'cpuPercent':cpuPercent,'cpuStat':cpuStat,'cpuFreq':cpuFreq,'vmem':vmem,'smem':smem,
'temp':temp,'fan':fan,'bat':bat,'processInfo':process_info,'process_info_cpuP':process_info_cpuP}

	with open("./Sys_Perf/"+taxi_id+".pickle","wb") as outfile:
		pickle.dump(sys_val,outfile)

	#save queue distribution
	with open("./queue_log/"+taxi_id+".json","w") as outfile:
		json.dump({"queue":request_queue},outfile)

	#save phermone matrix
	np.save("./Pheromone/"+taxi_id,pheromone)
...
if __name__ == '__main__':
	#initialize pheromone of taxi
	if(os.path.isfile("./Pheromone/"+taxi_id+".npy")):
		pheromone = np.load("./Pheromone/"+taxi_id+".npy")
	else:
		pheromone = np.random.rand(aco.n_cities,aco.n_cities)
	psutil.cpu_percent()
