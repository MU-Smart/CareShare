...
def find_min(pareto):
	...
	for i in pareto:
		print(i)
		if(math.sqrt(i["detour_obj"]**2 + i["time_obj"]**2)<m):
			m = math.sqrt(i["detour_obj"]**2 + i["time_obj"]**2)
			optimal = i
	return optimal 

def find_utility(pareto):
	list_all = []
	...
	for i in range(1,10):
		...
		for j in range(0,len(pareto)):
			u = (i/10 * pareto[j]["detour_obj"]) + ((10-i)/10 * pareto[j]["time_obj"])
			if(u<=val):
				val = u
				index = j

		list_all[index] += 1
	return pareto[np.argmax(list_all)]
...
def request(reqno):
	...
	if(len(taxi_ip)==0):
		...
		if(len(cost)>0):
			...
			temp_pareto_json["all"] = cost
			temp_pareto_json["pareto_front"] = pareto
			if(len(pareto)==0):
				cost.sort(key=operator.itemgetter('cost'))
				detour_obj = 'NA'
				time_obj = 'NA'
				temp_pareto_json["no_pareto"] = cost[0]
			else:
				#pareto_optimal = find_min(list(pareto))
				pareto_optimal = find_utility(list(pareto))
				cost = [pareto_optimal]
				print(cost)
				detour_obj = cost[0]["detour_obj"]
				time_obj = cost[0]["time_obj"]
				temp_pareto_json["multiple_pareto"] = cost[0]
...
with open('./Pareto/'+name+'.json', 'w') as outfile:
	json.dump(temp_pareto_json, outfile)
