...
def check_dest(pt,names,names_src,pass_array):
	for i in pass_array:
		if(str(i["dest"])==str(pt) and i["name"] not in names and i["name"] in names_src):
			return True,i["name"]
	return False,None
...
def broadcastToAllPeers(data):
    ...
    if((datetime.datetime.strptime(datetime.datetime.now().strftime("%H:%M:%S"),"%H:%M:%S")-(datetime.datetime.strptime(data["timestamp"],"%H:%M:%S"))).total_seconds() > (int(params['request_timers']) - 5)):
        ...
        return json.dumps({'id':taxi['id'], 'port': int(taxi_id), 'cost':-1})
    ...
    if(len(broadcast_firsthop_costs)==0):
        return({'cost':-1,'id' : taxi['id'], 'port':int(taxi_id)})
...
def processRequest():
    ...
    recieved = request.args
    print(datetime.datetime.strptime(datetime.datetime.now().strftime("%H:%M:%S"),"%H:%M:%S"),(datetime.datetime.strptime(recieved["timestamp"],"%H:%M:%S")))
    if((datetime.datetime.strptime(datetime.datetime.now().strftime("%H:%M:%S"),"%H:%M:%S")-(datetime.datetime.strptime(recieved["timestamp"],"%H:%M:%S"))).total_seconds() > int(params["request_timers"])):
       print("returning because :",(datetime.datetime.now()-(datetime.datetime.strptime(recieved["timestamp"],"%H:%M:%S"))).total_seconds())
       #no_messages += 1 #no_messages
       return json.dumps({'id':taxi['id'], 'port': int(taxi_id), 'cost':-1})
    ...
    if(request_in_process["status"]==True):
         ...
         if 'first_hop' in request.args:
             ...
             print("Passing to my neighbor because I am waiting for other passengers confirmation")
    ...
    retVal = {'id': taxi['id'], 'port':int(taxi_id), 'cost': recv_cost}
    ...
    #return value to passenger
    retVal = {'id': taxi['id'], 'port':int(taxi_id), 'cost': cost}
    ...
    if(check_pwtime_constraint() == False):
        no_messages += 1 #no_messages - message which returns back to customer (or taxi parent node - "first hop")
        if 'first_hop' in request.args:
        	return(json.dumps(retVal))
        else:
